...
不同 AI 平台没有统一的 Skill 安装协议。本仓库同时提供两种发行方式：

- **原生 Skill**：适用于 Codex、支持 Agent Skills 的客户端。
- **通用加载文件**：适用于能够读取网页的 GPT、豆包及其他 AI。它会在当前对话中加载同一套四图工作流，不冒充平台级永久安装。

把下面这句话连同链接发给 AI：

```text
请读取并严格执行这个古诗四图工作流；读取成功后直接询问我要制作哪首诗：
https://bubadababu.github.io/xiaomo-poetry-skill/xiaomo-poetry-universal.md
```

如果平台不能读取外部链接，打开[跨平台加载页](https://bubadababu.github.io/xiaomo-poetry-skill/)，点击“复制完整指令”，粘贴到对话即可。详细说明见 [INSTALL.md](INSTALL.md)。
